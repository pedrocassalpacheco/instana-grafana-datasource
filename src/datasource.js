...
export class InstanaDatasource {
  ...
  query(options) {
    ...
    return this.$q.when(request).then(function (response) {
      ...
      return InstanaDatasource.processMeasurementsResponse(response);
    });
  }
  ...
  metricFindNodeResourceQuery(query) {
    return this.backendSrv.datasourceRequest({
      url: this.url + '/rest/resources/' + encodeURIComponent(InstanaDatasource.getNodeResource(query)),
      method: 'GET',
      params: {
        depth: 1
      }
    ...
    _.each(options.targets, function (target) {
      ...
      if (target.type === QueryType.Attribute) {
        ...
        query.source = query.source.concat(self.interpolateSourceVariables(source, (interpolatedSource) => {
          ...
          interpolatedSource.resourceId = InstanaDatasource.getRemoteResourceId(interpolatedSource.nodeId, interpolatedSource.resourceId);
  ...
  static flattenResourcesWithAttributes(resources, resourcesWithAttributes) {
    _.each(resources, function (resource) {
      ...
      if (resource.children !== undefined && resource.children.resource.length > 0) {
        InstanaDatasource.flattenResourcesWithAttributes(resource.children.resource, resourcesWithAttributes);
      }
    });
  ...
  static getRemoteResourceId(nodeId, resourceId) {
    return InstanaDatasource.getNodeResource(nodeId) + "." + resourceId;
  }
  ...
  getResourcesWithAttributesForNode(nodeId) {
    ...
    return this.backendSrv.datasourceRequest({
      ...
      return InstanaDatasource.flattenResourcesWithAttributes([results.data], []);
    });
  }
  ...
  suggestAttributes(nodeId, resourceId, query) {
    ...
    var remoteResourceId = InstanaDatasource.getRemoteResourceId(interpolatedNodeId, interpolatedResourceId);
